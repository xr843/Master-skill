...
- **改小节标题前先搜谁指着它**：SKILL.md 的决策树和 Quick Reference 写
  `读 \`references/teaching.md\` §参话头`，模型照字面去找。`scripts/validate-section-references.py`
  在每个 PR 上核对这类指针：`§` 后的名字要出现在目标文件的某个标题或 `**粗体标签**` 里
  （忽略标点空格，括注可有可无），带目录的路径即使不带 `§` 也必须存在。
  2026-09-16 实测 198 条里 18 条落空，几乎都是标题改名后没回头改路由；最要紧的一条是
  米拉日巴把「拙火」问题指向 `grubum-excerpts.md` §拙火与气脉——该文件故意不收拙火修法，
  并在「本目录不收录之内容」里写明了应怎样回应。多个小节用顿号连写：`§一心三观、§六即佛`。
