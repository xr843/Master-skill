...
### Fixed — a required Fidelity check can no longer pass after grading nothing
- **`Fidelity smoke (1 master × 1 fixture)` was branch-protection-required but exited 0 whenever `ANTHROPIC_API_KEY` was absent — including on the main repository, where the secret has never been configured.** The emitted artifact said `{"skipped": true, "reason": "no_api_key"}`, while GitHub reported the required check green. Trusted events (same-repository PRs, pushes to `main`, scheduled runs, and manual runs) now fail loudly without the key. Only fork PRs retain an explicit advisory skip, because GitHub deliberately withholds repository secrets from untrusted forks.
- **The check built to catch this exact failure had never been connected to a real Fidelity result.** `check-gate-liveness.py` defined and unit-tested `check_graded_suites_graded_something()`, but `run_all()` never called it and the CLI accepted no result path; the production check could print “every gate examined a non-empty set” while the required smoke had examined nothing. `--fidelity-results` now accepts runner output or a committed report, rejects an empty result set, a dry-run result, or any graded suite with zero PASS/FAIL verdicts, and both paid workflow jobs invoke it on their artifact.
- **Same-repository PRs ran the whole workflow twice.** `push` matched every feature branch and `pull_request` matched the same commit again; PR #148 visibly carried two copies of every validation and Fidelity job. Once a key existed, every PR update would have paid for the same smoke twice. `push` is now restricted to `main`; feature branches run through `pull_request` exactly once.
