...
- Added `eval/reports/`, the first scored fidelity run ever committed. Previously `scripts/test-fidelity.py` only printed to stdout: 211 fixtures existed and not one recorded verdict did. The run stopped at 84/211 when the API account ran out of credit; the remaining 127 are recorded as **not measured**, never as failures. Headline **59/84 (70.2%)**, with the per-`test_type` split that turned out to matter far more than the aggregate: `fidelity` 89.6%, `boundary` 46.2%, `pressure` 40.0%. Zero fabricated citations **in the six cases where that audit actually ran** (see the retraction below). The persona *content* holds; the *guardrails* do not — and the guardrails are what `ETHICS.md` exists to guarantee.
