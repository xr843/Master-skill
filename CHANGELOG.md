...
- **The check built to catch this exact failure had never been connected to a real Fidelity result.** `check-gate-liveness.py` defined and unit-tested `check_graded_suites_graded_something()`, but `run_all()` never called it and the CLI accepted no result path; the production check could print “every gate examined a non-empty set” while the required smoke had examined nothing. `--fidelity-results` now accepts runner output or a committed report, rejects an empty result set or any graded suite with zero PASS/FAIL verdicts, and both paid workflow jobs invoke it on their artifact.
